...
-- Update get_source_counts to also normalize Asian sources
...
AS $$
  SELECT 
    CASE 
      ...
      WHEN r.source LIKE 'shopee%' THEN 'shopee'
      WHEN r.source LIKE 'lazada%' THEN 'lazada'
      WHEN r.source LIKE 'amazon%' THEN 'amazon'
      WHEN r.source LIKE 'web_review%' THEN 'web_review'
      ELSE r.source
    END AS source,
    COUNT(*) AS count
  FROM reviews r
