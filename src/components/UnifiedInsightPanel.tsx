...
interface Props {
  ...
  /** If true, hides raw-text evidence for privacy-restricted sources */
  privacyMode?: boolean;
}
...
function getSourceBreakdown(reviews: { source?: string }[]) {
  const counts: Record<string, number> = {};
  for (const r of reviews) {
    const key = getSourceLabel(r.source) || "기타";
...
/* ── Component ── */
export function UnifiedInsightPanel({ sentiment, productName, reviews, privacyMode = false }: Props) {
  ...
  return (
    <div className="gradient-card rounded-xl border border-border p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold font-heading">🔑 리뷰 인사이트 통합</h3>
        <div className="flex items-center gap-1.5">
          {sourceBreakdown.map(([src, cnt]) => (
            <span key={src} className="text-[9px] text-muted-foreground bg-muted/50 px-1.5 py-0.5 rounded">
              {src} {cnt}
            </span>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center">
        <div className="space-y-2">
          ...
          <p className="text-[9px] text-muted-foreground text-center font-sans">
            총 <span className="font-sans">{total.toLocaleString()}</span>건 감성 분석 · 감성 점수 <span className="font-sans font-bold">{sentiment.compositeScore}</span>/100
          </p>
        </div>
        ...
        {ratedCount > 0 && (
          <div className="flex flex-col items-center gap-1 px-3 py-2 rounded-lg border border-border/60 bg-muted/20 min-w-[120px]">
            <div className="flex items-center gap-1">
              <span className="text-lg font-bold font-sans text-foreground">{avgRating}</span>
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-1 text-[9px]">
                    <span className="w-3 text-right text-muted-foreground font-sans">{star}</span>
                    ...
                    <span className="w-6 text-right text-muted-foreground font-sans">{dist[star]}</span>
                  </div>
                );
              })}
            </div>
            <p className="text-[8px] text-muted-foreground"><span className="font-sans">{ratedCount}</span>건 평점 보유</p>
          </div>
        )}
      </div>

      {/* ② 긍정/부정 키워드 — Pros/Cons 태그 우선, 없으면 테마 라벨 */}
      ...
      <div className="flex items-center gap-1.5 text-[9px] text-muted-foreground pt-1 border-t border-border/50">
        ...
        <span>{productName} · {total.toLocaleString()}건{privacyMode ? " · 개인정보 보호 정책 준수" : ""}</span>
      </div>
    </div>
  );
}
