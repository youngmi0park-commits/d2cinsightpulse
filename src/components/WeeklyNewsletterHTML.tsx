...
function generatePreviewHTML(d: NewsletterData): string {
  ...
  const channelBadges = d.channels.map(ch => {
    return `<td style="padding:0 4px;"><!--[if mso]><table cellpadding="0" cellspacing="0" border="0"><tr><td style="border:1px solid ${BORDER};background:${WARM_BG};padding:5px 14px;font-size:12px;color:#555;font-weight:600;font-family:${FONT};mso-border-alt:solid ${BORDER} 1px;"><span style="font-size:6px;color:${ch.color};">&#9679;</span> ${ch.name} <strong style="color:#333;">${ch.count.toLocaleString()}</strong></td></tr></table><![endif]--><!--[if !mso]><!--><table cellpadding="0" cellspacing="0" border="0"><tr><td style="border:1px solid ${BORDER};background:${WARM_BG};padding:5px 14px;font-size:12px;color:#555;font-weight:600;font-family:${FONT};border-radius:20px;"><span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${ch.color};margin-right:5px;vertical-align:middle;"></span>${ch.name} <strong style="color:#333;">${ch.count.toLocaleString()}</strong></td></tr></table><!--<![endif]--></td>`;
  }).join("");
  ...
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:${WARM_BG};">
    <tr><td style="padding:20px 40px;font-family:${FONT};">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
        ...
        <tr><td style="font-size:12px;color:#666;line-height:22px;">RTA Studio는 15개국, 43개+ 채널의 실사용자 리뷰를 통합 분석하여 숨겨진 인사이트를 발견하고, 즉시 활용 가능한 마케팅 에셋을 제공하는 올인원 플랫폼입니다.</td></tr>
      </table>
    </td></tr>
  </table>
</td></tr>
